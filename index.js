...
let playerHorizSpace = rectArea.width - (rectPlayer.width + 25);
...
function startGame() {
	if (gameRunning == false) {
		...
		alienSpeed = 7 + alienSpeedLevelup;
...
function gameLoop() {
	...
	for (let i = bullets.length - 1; i >= 0; i--) {
		//to catch console error if 'endGame' is called with bullets still on screen
		if (gameRunning == false) {
			return;
		}
		// if they exit screen OR if they collide, then remove div
			...
			for (let j = aliens.length - 1; j >= 0; j--) {
				...
				if (rectBullet.x < rectAlien.x + rectAlien.width && rectBullet.x + rectBullet.width > rectAlien.x && rectBullet.y < rectAlien.y + rectAlien.height && rectBullet.y + rectBullet.height > rectAlien.y) {
